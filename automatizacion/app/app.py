...
def update_dispositivo(id):
    ...
    data = request.get_json()
    for key in ['nombre', 'ip', 'tipo', 'ubicacion', 'username', 'contrasenia', 'enable_secret', 'estado']:
        if key in data:
            setattr(dispositivo, key, data[key])
    db.session.commit()
...
@login_required
def lista_dispositivos():
    dispositivos = Dispositivo.query.all()
    return render_template('crud/dispositivos.html', dispositivos=dispositivos)
...
def nuevo_dispositivo():
    if request.method == 'POST':
        nuevo = Dispositivo(
            nombre=request.form['nombre'],
            ip=request.form['ip'],
            tipo=request.form.get('tipo', 'cisco_ios'),
        ...
        return redirect(url_for('lista_dispositivos'))

    return render_template('crud/nuevo_dispositivo.html')
...
def editar_dispositivo(id):
    ...
    if request.method == 'POST':
        ...
        dispositivo.username = request.form['username']
        dispositivo.contrasenia = request.form['contrasenia']
        dispositivo.enable_secret = request.form['enable_secret']
    ...
    return render_template('crud/editar_dispositivo.html', dispositivo=dispositivo)
...
# Servicio SOAP para el modelo Dispositivo
class DispositivoSOAP(ServiceBase):

    @rpc(Integer, _returns=Unicode)
    def getDispositivo(ctx, id):
        with app.app_context():
            d = Dispositivo.query.get(id)
            if d:
                return f"ID: {d.id} | Nombre: {d.nombre} | IP: {d.ip} | Estado: {d.estado} | Ubicacion: {d.ubicacion}"
            return " Dispositivo no encontrado"

    @rpc(Unicode, Unicode, Unicode, Unicode, _returns=Unicode)
    def addDispositivo(ctx, nombre, ip, tipo, ubicacion):
        with app.app_context():
            try:
                nuevo = Dispositivo(nombre=nombre, ip=ip, tipo=tipo, ubicacion=ubicacion)
                db.session.add(nuevo)
                db.session.commit()
                return f" Dispositivo agregado con ID: {nuevo.id}"
            except Exception as e:
                return f" Error al agregar: {str(e)}"

    @rpc(Integer, Unicode, _returns=Unicode)
    def updateUbicacion(ctx, id, nueva_ubicacion):
        with app.app_context():
            d = Dispositivo.query.get(id)
            if d:
                d.ubicacion = nueva_ubicacion
                db.session.commit()
                return f" Ubicación actualizada {d.ubicacion}"
            return " Dispositivo no encontrado"

    @rpc(Integer, _returns=Unicode)
    def deleteDispositivo(ctx, id):
        with app.app_context():
            d = Dispositivo.query.get(id)
            if d:
                db.session.delete(d)
    ...
    def getAllDispositivos(ctx):
        with app.app_context():
            print("TOTAL DISP:", Dispositivo.query.count())  # DEBUG
            dispositivos = Dispositivo.query.all()
            return [
                f"ID: {d.id} | Nombre: {d.nombre} | IP: {d.ip} | Estado: {d.estado} | Ubicacion: {d.ubicacion}"
                for d in dispositivos
            ]
...
# Integrar el servicio SOAP a la app Flask
app.wsgi_app = DispatcherMiddleware(app.wsgi_app, {
    '/soap': soap_wsgi_app
})

@app.route("/soap_ui", methods=["GET", "POST"])
@login_required
@csrf.exempt
def soap_ui():
    return render_template("crud/soap_ui.html")

@app.route("/debug_agregar")
def debug_agregar():
    with app.app_context():
        # Borra todos los dispositivos anteriores
...
@app.route("/ver_dispositivos_directo")
def ver_dispositivos_directo():
    with app.app_context():
        dispositivos = Dispositivo.query.all()
        return "<br>".join(
            f"{d.id}: {d.nombre} - {d.ip} - {d.estado} - {d.ubicacion}" for d in dispositivos
        )
